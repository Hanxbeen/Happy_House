...
public class VillaDaoImpl implements VillaDao {
	...
	private static VillaDao villaDao;
	
	private VillaDaoImpl() {}
	
	public static VillaDao getVillaDao() {
		if(villaDao == null)
			villaDao = new VillaDaoImpl();
		return villaDao;
	}
	...
	public List<VillaInfoDto> getDongInGugun(String gugun) throws Exception {
		...
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<VillaInfoDto> list = new ArrayList<VillaInfoDto>();
		try {
			conn = DBUtil.getConnection();
			StringBuilder sql = new StringBuilder();
			sql.append("SELECT distinct dong FROM villadeal \n");
			...
			rs = pstmt.executeQuery();
			while(rs.next()) {
				VillaInfoDto dto = new VillaInfoDto();
	...
	public List<VillaInfoDto> getVillaInDong(String dong) throws Exception {
		...
		try {
			conn = DBUtil.getConnection();
			StringBuilder sql = new StringBuilder();
			sql.append("SELECT amount,area,bunzi,dong,floor,name,villaid FROM villadeal WHERE dong = ? \n");
			...
			while(rs.next()) {
				...
				dto.setVillaid(rs.getInt("villaid"));
				list.add(dto);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	@Override
	public List<VillaInfoDto> getVillaName(String VillaName) throws Exception {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		List<VillaInfoDto> list = new ArrayList<VillaInfoDto>();
		try {
			conn = DBUtil.getConnection();
			StringBuilder sql = new StringBuilder();
			sql.append("SELECT amount,area,bunzi,dong,floor,name,villaid FROM villadeal WHERE name Like ? \n");
			sql.append("ORDER BY name");
			pstmt = conn.prepareStatement(sql.toString());
			pstmt.setString(1,"%"+ VillaName + "%");
			rs = pstmt.executeQuery();
			while(rs.next()) {
				VillaInfoDto dto = new VillaInfoDto();
				dto.setAmount(rs.getString("amount"));
	...
	public List<VillaInfoDto> getAllVilla() throws Exception {
		...
		try {
			conn = DBUtil.getConnection();
			StringBuilder sql = new StringBuilder();
			sql.append("SELECT amount,area,bunzi,dong,floor,name,villaid FROM villadeal \n");
			sql.append("ORDER BY name");
			pstmt = conn.prepareStatement(sql.toString());
			rs = pstmt.executeQuery();
			while(rs.next()) {
